...
def solve_ivp(fun, t0, t1, dt, y0):
    t = t0
    y = y0
    ys = jax.tree.map(lambda x: np.array([x]), y)
    times = [t]
    while t < t1:
        y = rk4(fun, t, y, dt)
        t = t + dt
        ys = jax.tree.map(lambda x, y: np.concatenate([x, np.asarray([y])]), ys, y)
        times.append(t)
...
sol1 = np.asarray(sol1)  # [..., :3500]
sol2 = np.asarray(sol2)  # [..., :3500]
times = times  # [:3500]
...
ax1.plot(sol1[0], sol1[1], sol1[2], label='With Quantities', color='blue')
ax1.plot(sol2[0], sol2[1], sol2[2], label='Without Quantities', color='red', alpha=0.7)
...
plt.savefig(f'results/lorenz_en_quantities-{dtype.__name__}.pdf')
plt.savefig(f'results/lorenz_en_quantities-{dtype.__name__}.eps')
