...
class LifRefLTC(bst.nn.Neuron):
  def __init__(
    ...
    # initialization
    super().__init__(size, name=name, keep_size=keep_size, mode=mode, )
  ...
  def update(self, x=0 * u.mA):
    ...
    I = self.sum_current_inputs(self.V.value, init=x)
    dv = (-self.V.value + self.V_rest + self.R * I) / self.tau
    V = self.V.value + dv * bst.environ.get_dt() + self.sum_delta_inputs(init=0. * u.mV)
...
class Expon(bst.nn.Synapse):
  ...
  def update_return_info(self):
    return jax.ShapeDtypeStruct(self.varshape, bst.environ.dftype()) * u.siemens

  def update_return(self):
    return self.g.value


class EventCSRLinear(bst.Module):
  def __init__(
      self,
      weight,
      prob: float,
  ...
  def update(self, x):
    ...
    if isinstance(self.weight, u.Quantity):
      ...
      w = self.weight
    r = bm.jitconn.event_mv_prob_homo(x, w, self.prob, self.seed, shape=(self.n_pre, self.n_post), transpose=True,
...
class VisualSystem(bst.ModuleGroup):
  def __init__(
...
def pop_expon_syn(pre, post, delay, prob, g_max, tau):
  return bst.nn.FullProjAlignPostMg(pre, delay, EventCSRLinear(g_max, prob, pre.num, post.num),
                                    Expon.delayed(post.num, tau=tau), CUBA.delayed(), post)
...
def sps_show(area_names, sps, run_indices, num_exc, title):
  fig, gs = bts.visualize.get_figure(1, 1, 4.5, 6.)
...
def _create_model(g_max):
  ...
  flnMatp = bts.file.load_matfile('Joglekar_2018_data/efelenMatpython.mat')
  conn = np.asarray(flnMatp['flnMatpython'].squeeze())  # fln values..Cij is strength from j to i

  # Distance
  speed = 3.5  # axonal conduction velocity
  distMatp = bts.file.load_matfile('Joglekar_2018_data/subgraphWiring29.mat')
...
def try_large_scale_system():
  ...
  duration = 100.
  duration = 70.
  v1_inputs = bts.input.section_input([0. * u.mA, 10. * u.mA, 0. * u.mA],
                                      [200. * u.ms, duration * u.ms, 500. * u.ms])
  bg_inputs = u.math.ones_like(v1_inputs) * 10.5
  run_indices = np.arange(v1_inputs.size)
  outs = bst.transform.for_loop(model.step_run, run_indices, bg_inputs, v1_inputs, pbar=bst.transform.ProgressBar(10))
  t1 = time.time()
  ...
  fig, gs = bts.visualize.get_figure(1, 1, 4.5, 6.)
  fig.add_subplot(gs[0, 0])
  plt.plot(run_indices, bts.metric.firing_rate(outs['E-sps'], 10., dt=0.1), label='E')
  plt.plot(run_indices, bts.metric.firing_rate(outs['I-sps'], 10., dt=0.1), label='I')
  plt.legend()
  plt.show()

  np.savez(
    f'results/spikes-gIE={g_max[2].mantissa:.5f}-muEE={g_max[4].mantissa:.5f}-duration={duration}.npz',
    E_sps=np.asarray(outs['E-sps']), I_sps=np.asarray(outs['I-sps']),
    g_max=np.asarray(g_max.mantissa),
  )
...
def try2():
  ...
  g_max = np.asarray([0.10108301, 0.60604239, -0.60977116, -0.33540355, 0.075]) * u.siemens
  model = _create_model(g_max)
  v1_inputs = bts.input.section_input([0. * u.mA, ], [100. * u.ms, ])
  bg_inputs = u.math.ones_like(v1_inputs) * 10.5
  run_indices = np.arange(v1_inputs.size)
  ...
  def run():
    outs = bst.transform.for_loop(model.step_run, run_indices, bg_inputs, v1_inputs)
