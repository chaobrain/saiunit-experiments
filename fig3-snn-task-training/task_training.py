...
class SignedWLinear(bst.nn.Module):
    ...
    def __init__(
        ...
        self.weight = bst.ParamState(weight)
...
class SNNNet(bst.nn.Module):
    ...
    def visualize(self, inputs, n2show: int = 5, filename: str = None):
        ...
        res = bst.compile.for_loop(step, indices, inputs, pbar=bst.compile.ProgressBar(10))
...
class SNNCobaNet(SNNNet):
    def __init__(
        ...
        self.ff2r = bst.nn.AlignPostProj(
            ...
            syn=Expon.desc(size=n_rec, tau=tau_syn),
        ...
        self.inh2r = bst.nn.AlignPostProj(
            comm=inh2r_conn,
            syn=Expon.desc(in_size=n_rec, tau=tau_syn),
...
class Trainer:
    ...
    def bptt_train(self, inputs, targets) -> Tuple:
        ...
        bst.nn.init_all_states(self.target_net, inputs.shape[1])
        weights = self.target_net.states().subset(bst.ParamState)
        ...
        def _bptt_grad():
            (mse_losses, l1_losses), outs = bst.compile.for_loop(_step_run, indices, inputs)
        ...
        f_grad = bst.augment.grad(_bptt_grad, grad_vars=weights, has_aux=True, return_value=True)
